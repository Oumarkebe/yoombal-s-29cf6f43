...
export function CategoryBadge({
  ...
  return (
    ...
    >
      <span
        className={`${isMini ? 'text-xs' : 'text-sm'} font-medium transition-colors ${isActive ? 'text-white' : 'text-slate-700 group-hover:text-slate-900'}`}
      >
        {name}
      </span>
      {showIcon && !isMini && (
        <Icon
          className={`w-4 h-4 transition-transform group-hover:scale-110 ${isActive ? 'text-white' : theme.color}`}
        />
      )}
    </button>
  );
