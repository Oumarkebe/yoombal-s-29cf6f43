...
const MerchantStats = ({ orders: propOrders, enrichedOrders: propEnrichedOrders }: any) => {
  ...
  React.useEffect(() => {
    ...
    const enrichOrders = async () => {
      const enriched = await Promise.all(
        (propOrders || orders).map(async (order: any) => {
          let items = [];
          try {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'processing':
        return (
          <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">En cours</span>
        );
      case 'completed':
      case 'delivered':
        return (
      ...
      default:
        return (
          <span className="px-2 py-1 bg-gray-100 text-gray-800 rounded-full text-xs">Inconnu</span>
        );
    }
  };
            ...
            <div>
              ...
              <div className="flex items-center gap-1 mt-2">
                ...
                <span className="text-sm text-green-600">
                  {salesData[4]?.ventes
                    ? `${Math.round(((salesData[5]?.ventes - salesData[4]?.ventes) / (salesData[4]?.ventes || 1)) * 100)}% vs mois dernier`
                    : ''}
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-1 mt-2">
                <TrendingUp className="h-4 w-4 text-green-600" />
                <span className="text-sm text-green-600">
                  {salesData[4]?.commandes
                    ? `${Math.round(((salesData[5]?.commandes - salesData[4]?.commandes) / (salesData[4]?.commandes || 1)) * 100)}% vs mois dernier`
                    : ''}
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">
                {salesData[5]?.commandes
                  ? formatCurrency((salesData[5]?.ventes || 0) / (salesData[5]?.commandes || 1))
                  : 'N/A'}
              </p>
              <div className="flex items-center gap-1 mt-2">
                ...
                <span className="text-sm text-red-600">
                  {salesData[4]?.commandes
                    ? `${Math.round((((salesData[5]?.ventes || 0) / (salesData[5]?.commandes || 1) - (salesData[4]?.ventes || 0) / (salesData[4]?.commandes || 1)) / ((salesData[4]?.ventes || 0) / (salesData[4]?.commandes || 1) || 1)) * 100)}% vs mois dernier`
                    : ''}
                </span>
              </div>
            </div>
