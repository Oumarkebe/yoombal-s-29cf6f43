...
const fetchOrderDetails = async (orderId: string) => {
  ...
  const [orderRes, itemsRes] = await Promise.all([
    (supabase.from('admin_orders_view' as any) as any).select('*').eq('id', orderId).single(),
    supabase
      .from('order_items')
...
export function OrderDetailsDialog({ orderId, onClose }: OrderDetailsDialogProps) {
  ...
  const handleUpdateStatus = async () => {
    ...
    setIsUpdating(true);
    try {
      const { error } = await supabase.from('orders').update({ status: status }).eq('id', orderId);
            ...
            <div>
              ...
              <div className="space-y-3">
                {order.order_items.map((item: any) => (
                  <div
                    key={item.id}
                    className="flex gap-3 items-center border p-2 rounded-lg bg-gray-50"
                  >
                    <img
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-500">
                        qté: {item.quantity} x {formatCurrency(item.unit_price)}
                      </div>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <div className="border-t pt-4">
              <Label className="mb-2 block font-semibold">
                Mise à jour du statut (Supervision)
              </Label>
