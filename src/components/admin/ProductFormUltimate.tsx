...
export function ProductFormUltimate({
  initialData,
  onSubmit,
  isLoading,
  onClose,
}: ProductFormUltimateProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50/50">
      <div className="flex-1 overflow-y-auto px-1">
        {' '}
        {/* Padding minimal pour éviter scrollbar coupée */}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {formData.compare_at_price && formData.compare_at_price > formData.price ? (
                        <p className="text-xs text-green-600 font-medium">
                          -{Math.round((1 - formData.price / formData.compare_at_price) * 100)}% de
                          réduction
                        </p>
                      ) : null}
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4 border-t">
                    ...
                    <div className="space-y-2 bg-slate-50 p-3 rounded-lg border border-slate-100">
                      ...
                      <div className="flex justify-between items-center text-sm pt-1">
                        ...
                        <span
                          className={`font-bold ${calculateMargin() >= 30 ? 'text-green-600' : 'text-amber-600'}`}
                        >
                          {calculateMargin().toFixed(1)}% ({formData.price - formData.cost_price}{' '}
                          CFA)
                        </span>
                      </div>
                    </div>
            ...
            <TabsContent value="media" className="mt-0 pt-4">
              <MediaUploader
                ...
                onVideoChange={(video_url) =>
                  setFormData({ ...formData, video_url: video_url || null })
                }
              />
            </TabsContent>
            ...
            <TabsContent value="seo" className="mt-0 pt-4">
              <SEOAndIAForm formData={formData} onChange={setFormData} />
            </TabsContent>

            {/* ONGLET 6: Avancé (B2B & Digital) */}
            <TabsContent value="advanced" className="mt-0 pt-4">
              <AdvancedSettingsForm formData={formData} onChange={setFormData} />
            </TabsContent>
          </Tabs>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 flex justify-between items-center z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-2">
              <Switch
                ...
                onCheckedChange={(c) =>
                  setFormData({ ...formData, published_at: c ? new Date().toISOString() : null })
                }
              />
              <Label className="cursor-pointer">
                {formData.published_at ? (
                  ...
                  <span className="text-gray-500">Brouillon (Hors ligne)</span>
                )}
              </Label>
            </div>
            <div className="flex gap-2">
