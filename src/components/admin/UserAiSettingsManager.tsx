...
const AI_FEATURES_CONFIG: { key: AiFeatureKey; label: string; description: string }[] = [
  {
    ...
    description: "Autoriser la génération de descriptions de produits par l'IA.",
  },
  {
    key: 'pricing',
    label: 'Pricing Dynamique',
    description: "Activer les suggestions de prix basées sur l'IA.",
  },
  {
    key: 'predictions',
    label: 'Analyses Prédictives',
    description: 'Fournir des prédictions de ventes et de tendances.',
  },
  {
    key: 'ai_assistant',
    label: 'Assistant IA',
    description: "Activer le chatbot d'assistance intelligent pour le marchand.",
  },
  {
    key: 'ai_vision',
    label: 'Vision IA',
    description: "Activer le contrôle qualité des images et l'analyse visuelle.",
  },
  {
    key: 'ai_smart_search',
    label: 'Recherche Intelligente',
    description: 'Activer la recherche vocale et visuelle pour les clients.',
  },
];
        ...
        return (
          <div key={key} className="flex items-start justify-between">
            <div className="flex-1 pr-4">
              <Label htmlFor={`ai-${key}`} className="font-medium">
                {label}
              </Label>
              <p className="text-xs text-gray-500">{description}</p>
              {userOverride === undefined && (
                <p className="text-xs text-blue-500 italic mt-1">
                  Utilise le paramètre global (actuellement {isChecked ? 'activé' : 'désactivé'})
                </p>
              )}
              {userOverride !== undefined && (
                <p className="text-xs text-purple-600 italic mt-1">
                  Paramètre spécifique appliqué.
                </p>
              )}
            </div>
            <Switch
