...
import { Loader2, Shield, CreditCard, Sparkles, AlertTriangle, Save, CheckCircle } from 'lucide-react';
...
export function UserDetailModal({ userId, isOpen, onClose }: UserDetailModalProps) {
    ...
    const { data: featureStates, isLoading: isFeaturesLoading, refetch: refetchFeatures } = useQuery({
    ...
    const renderFeatureRow = (state: EffectiveFeatureState) => {
        return (
            <div key={state.feature.feature_key} className="flex items-center justify-between p-3 border rounded-lg hover:bg-slate-50 transition-colors">
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        <p className="font-medium text-sm text-gray-900">{state.feature.name}</p>
                        {state.feature.is_premium && <Badge variant="secondary" className="text-[10px] h-5">Premium</Badge>}
                        {!state.feature.is_enabled && <Badge variant="destructive" className="text-[10px] h-5">Désactivé Globalement</Badge>}
                    </div>
                    <p className="text-xs text-gray-500 line-clamp-1">{state.feature.description}</p>
                    <div className="flex items-center gap-2 text-xs">
                        <span className="text-gray-400">Source:</span>
                        {state.source === 'plan' && <span className="text-blue-600 font-medium">Inclus dans le plan</span>}
                        {state.source === 'global_off' && <span className="text-red-500 font-medium">Global Admin OFF</span>}
                        {state.source === 'override_on' && <span className="text-green-600 font-bold flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Forcé ON</span>}
                        {state.source === 'override_off' && <span className="text-amber-600 font-bold flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Forcé OFF</span>}
                ...
                <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-md">
                    <Button
                        ...
                        onClick={() => toggleFeatureMutation.mutate({ key: state.feature.feature_key, value: true })}
                    ...
                    <Button
                        ...
                        onClick={() => toggleFeatureMutation.mutate({ key: state.feature.feature_key, value: null })}
                    ...
                    <Button
                        ...
                        onClick={() => toggleFeatureMutation.mutate({ key: state.feature.feature_key, value: false })}
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col p-0 overflow-hidden">
                <DialogHeader className="px-6 py-4 border-b">
                    ...
                    <DialogDescription>
                        {isProfileLoading ? "Chargement..." : `${profile?.first_name || 'Utilisateur'} ${profile?.last_name || ''}`}
                    ...
                    <Tabs defaultValue="profile" className="flex-1 flex flex-col" value={activeTab} onValueChange={setActiveTab}>
                        ...
                        <div className="flex-1 overflow-hidden bg-slate-50/50 p-6">
                            <TabsContent value="profile" className="h-full m-0 space-y-6">
                                {isProfileLoading ? <div className="flex justify-center py-10"><Loader2 className="animate-spin text-gray-400" /></div> : (
                                    <div className="space-y-6">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-2">
                                                <Label>Statut du compte</Label>
                                                <div className="flex items-center gap-2">
                                                    <Badge variant={profile?.status === 'active' ? 'default' : 'destructive'} className="uppercase">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-xs text-muted-foreground">Attention: donner le rôle Admin donne accès complet.</p>
                                            </div>
                                        </div>
                                        <Separator />
                                        <div className="space-y-2">
                                ...
                                <div className="bg-white p-6 rounded-lg border shadow-sm space-y-6">
                                    <div className="flex items-start justify-between">
                                        ...
                                        {subscription ? (
                                            ...
                                            <Badge variant="outline" className="text-gray-500">Aucun abonnement (Gratuit)</Badge>
                                        )}
                                    </div>
                                    ...
                                    {isFeaturesLoading ? (
                                        ...
                                        <div className="space-y-6 pb-6">
                                            {Object.entries(groupedFeatures).map(([category, items]) => (
                                                <div key={category}>
                                                    <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2 pl-1">{category}</h4>
                                                    <div className="space-y-2">
