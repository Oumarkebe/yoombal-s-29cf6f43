...
import { CalendarIcon, Package, Download, Building2, Upload, Loader2, CheckCircle2 } from 'lucide-react';
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Switch
                            ...
                            onCheckedChange={(checked) =>
                                onChange({ ...formData, is_digital: checked })
                            }
                        />
                    </div>
                        ...
                        <div className="space-y-4 pt-4 border-t mt-2">
                            <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={async (e) => {
                                                ...
                                                try {
                                                    ...
                                                    toast.error(`Erreur lors de l'envoi : ${error.message}`, { id: loadingToast });
                                                }
                                            }}
                                        />
                                        <Button
                                ...
                                <p className="text-sm text-muted-foreground mt-1">
                                    Vous pouvez coller un lien existant ou cliquer sur "Héberger" pour envoyer un fichier sur nos serveurs sécurisés.
                                </p>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="space-y-2">
                            ...
                            {formData.wholesale_price && formData.price && (
                                <p className="text-xs text-blue-600 font-medium">
                                    Marge grossiste: -{Math.round((1 - formData.wholesale_price / formData.price) * 100)}% sur le prix public
                                </p>
                            )}
                        </div>
                            ...
                            <div className="space-y-2 md:col-span-2">
                                ...
                                <div className="flex gap-2">
                                    <Input
                                        ...
                                        onChange={(e) => onChange({ ...formData, dimensions: { ...formData.dimensions, length: parseInt(e.target.value) || 0 } })}
                                    />
                                    <Input
                                        placeholder="l"
                                        type="number"
                                        value={formData.dimensions?.width || ''}
                                        onChange={(e) => onChange({ ...formData, dimensions: { ...formData.dimensions, width: parseInt(e.target.value) || 0 } })}
                                    />
                                    <Input
                                        placeholder="h"
                                        type="number"
                                        value={formData.dimensions?.height || ''}
                                        onChange={(e) => onChange({ ...formData, dimensions: { ...formData.dimensions, height: parseInt(e.target.value) || 0 } })}
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {formData.published_at && new Date(formData.published_at) > new Date() && (
                            <div className="bg-yellow-50 text-yellow-800 p-2 rounded text-sm border border-yellow-200">
                                Le produit sera visible automatiquement à partir du {format(new Date(formData.published_at), "Pp", { locale: fr })}.
                            </div>
                        )}
                    </div>
                </CardContent>
