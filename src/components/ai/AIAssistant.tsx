...
export function AIAssistant() {
  ...
  const assistantConfig =
    globalFeatures?.find(
      (f) => f.feature_key === 'ai_assistant' || f.feature_key === 'assistant_intelligent'
    )?.configuration || {};
  ...
  const syncToDb = async () => {
    if (!user) return;
    try {
      await (supabase as any).from('ai_chat_sessions').upsert(
        [
          {
            user_id: user.id,
        ...
        try {
          ...
          if (session.speak) {
            session
              ...
              .catch((e: any) => console.error('Piper speak error', e));
          } else if (session.predict) {
            session.predict({ text: cleanText }).then((res: any) => console.log('Piper predicted', res));
          }
          return;
        } catch (e) {
  ...
  const handleSendMessage = async (overrideInput?: string) => {
    ...
    try {
      const systemPrompt =
        (assistantConfig as any)?.system_prompt ||
        'Tu es un assistant utile pour Yoombal, une plateforme e-commerce sénégalaise.';
      ...
      if (!isEnabled) {
        finalSystemPrompt +=
          " IMPORTANT : L'utilisateur n'a pas d'abonnement premium. Tu ne peux PAS ajouter de produits au panier ou effectuer des commandes. Propose uniquement des conseils, des informations et aide-le à choisir. Suggère poliment de s'abonner pour activer les actions automatiques.";
      }
      ...
      if (data?.response) {
        ...
        const intentInfo = {
          ...
          confidence: parseFloat(
            data.response.match(/intent_confidence\s*:\s*([\d.]+)/)?.[1] || '1.0'
          ),
        };
        ...
        const policyCtx: AIContext = {
          user: {
            ...
            authorityOverride: userAuthorityLevel,
          },
          session: {
        ...
        if (decision.allowed) {
          ...
          if (intentInfo.type === 'add_cart' && intentInfo.targetId) {
            ...
            toast.success('Produit ajouté ! 🛒', {
              description:
                decision.requiredLevel === AIActionLevel.L3
        ...
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: "Désolé, j'ai rencontré une erreur technique." },
        ]);
      }
    } catch (err: any) {
      clearInterval(interval);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {/* Chat Window */}
      {isOpen && (
        <Card className="mb-4 w-[350px] sm:w-[400px] h-[500px] shadow-2xl flex flex-col border-indigo-100 overflow-hidden animate-in slide-in-from-bottom-5 duration-300">
          <CardHeader className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-4 flex flex-row items-center justify-between space-y-0">
                ...
                <div className="flex items-center gap-1.5 text-[10px] opacity-90">
                  <div
                    className={cn(
                      ...
                      isEnabled ? 'bg-green-400' : 'bg-blue-400'
                    )}
                  />
                  <span>
                    {isEnabled ? 'En ligne • Premium AI' : 'En ligne • Mode Conseil (L1)'}
                  </span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-1">
              ...
              {isPremiumLoading && <Loader2 className="w-3 h-3 text-white animate-spin mr-1" />}
              ...
              <Button
                ...
                className={cn(
                  ...
                  voiceEnabled ? 'opacity-100' : 'opacity-50'
                )}
                onClick={() => {
                        ...
                        <div className="p-3 flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-slate-500">
                              ID: {lastAnalyticTags.target_id?.slice(0, 8)}...
                            </p>
                          </div>
                        </div>
                      ...
                      <Card className="border-blue-200 bg-white shadow-sm overflow-hidden max-w-[280px]">
                        <div className="p-3 flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-slate-500">
                              Zone: {lastAnalyticTags.target_id || 'Dakar'}
                            </p>
                          </div>
                        </div>
                        <div className="bg-blue-50 p-2 text-center">
                          <span className="text-sm font-bold text-blue-700">
                            {calculateDeliveryFee(
                              getZoneByArea(lastAnalyticTags.target_id || 'Dakar')?.id || ''
                            )}{' '}
                            FCFA
                          </span>
                        </div>
