      ...
      <CardContent className="p-4">
        ...
        {status === 'idle' && (
          <p className="text-xs text-amber-600/80">
            L'IA peut analyser votre image pour s'assurer qu'elle respecte les standards
            professionnels.
          </p>
        )}
        ...
        {(status === 'passed' || status === 'failed') && (
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-full ${status === 'passed' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}
              >
                {status === 'passed' ? (
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <span
                    className={`text-sm font-bold ${status === 'passed' ? 'text-green-700' : 'text-red-700'}`}
                  >
