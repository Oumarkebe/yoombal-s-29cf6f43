...
const AI_FEATURES_CONFIG: { key: AiFeatureKey; label: string; description: string }[] = [
  {
    ...
    description: "Permet à l'IA de générer des descriptions de produit pour vous.",
  },
  {
    key: 'pricing',
    label: 'Pricing Dynamique',
    description: 'Suggestions de prix IA (si activé par la plateforme).',
  },
  {
    key: 'predictions',
    label: 'Analyses Prédictives',
    description: 'Accès aux statistiques avancées IA (si activé par la plateforme).',
  },
];
...
export function MyAiSettingsManager() {
  ...
  const {
    ...
    updateSetting,
    isUpdating,
  } = useUserAiSettings({ userId: user?.id });
        ...
        return (
          ...
          >
            <div className="flex-1 pr-6">
              <Label htmlFor={`ai-pro-${key}`} className="font-medium">
                {label}
              </Label>
