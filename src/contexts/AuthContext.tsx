...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const resetPassword = async (email: string) => {
    ...
    console.log('--- RESET PASSWORD DIAGNOSTIC ---');
    ...
    try {
      ...
      try {
        ...
        if (rpcError) {
          ...
          console.log('✅ RPC Result:', exists);
          if (exists === false) {
            console.warn('Blocked: Email not found in DB');
            return { error: "Aucun compte n'est associé à cet email." };
          }
        }
      } catch (e) {
        console.error('Unexpected error during RPC call:', e);
      }

      // 2. Standard reset call
      console.log('Proceeding to Supabase resetPasswordForEmail...');
