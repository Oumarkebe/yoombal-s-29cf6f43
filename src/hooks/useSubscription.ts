...
export function useSubscription() {
  ...
  const hasFeature = (featureKey: string): boolean => {
    ...
    if (featureKey === 'finance') {
      keysToCheck.push('fraud_detection', 'blanchiment_detection', 'audit_securite');
    }
    ...
    const checkInList = (list: string[]) => {
      ...
      if (list.includes('all_pro_features')) {
        ...
        if (
          proPlan?.features &&
          keysToCheck.some((k) => (proPlan.features as string[]).includes(k))
        ) {
          return true;
        }
      }
  ...
  const resolvedFeatures = React.useMemo(() => {
      ...
      globalFeatures.forEach((f) => {
        ...
        if (
          [
            ...
            'ai_vision',
            'vision_ai',
          ].includes(f.feature_key)
        ) {
          adminFeatures.add('ai_assistant');
        } else if (
          ['tarification_dynamique', 'ai_pricing', 'ramadan_pricing'].includes(f.feature_key)
        ) {
          ...
          ['fraud_detection', 'blanchiment_detection', 'audit_securite'].includes(f.feature_key)
        ) {
    ...
    const planFeatures =
      subscription?.plan?.features || plans.find((p: any) => p.slug === 'starter')?.features || [];
    ...
    activeSet.forEach((key) => {
      ...
      if (
        ...
        ['tarification_dynamique', 'ai_pricing', 'ramadan_pricing'].includes(key as string)
      ) {
        ...
        ['fraud_detection', 'blanchiment_detection', 'audit_securite'].includes(key as string)
      ) {
