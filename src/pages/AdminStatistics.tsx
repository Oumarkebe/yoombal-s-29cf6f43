...
import { Loader2, Users, Package, ShoppingCart, DollarSign, ArrowLeft, AlertTriangle } from 'lucide-react';
...
export const fetchAdminStats = async () => {
  ...
  const { data: orders, error: ordersError } = await supabase.from('orders').select('total_amount, created_at');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {someStatIsVisible ? (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
            {settings?.dashboard?.showUserCount && <StatCard title="Utilisateurs" value={data?.userCount ?? 0} icon={<Users className="h-5 w-5 text-blue-500" />} />}
            {settings?.dashboard?.showProductCount && <StatCard title="Produits" value={data?.productCount ?? 0} icon={<Package className="h-5 w-5 text-green-500" />} />}
            {settings?.dashboard?.showOrderCount && <StatCard title="Commandes" value={data?.orderCount ?? 0} icon={<ShoppingCart className="h-5 w-5 text-orange-500" />} />}
            {settings?.dashboard?.showTotalRevenue && <StatCard title="Revenu Total" value={formatCurrency(data?.totalRevenue ?? 0)} icon={<DollarSign className="h-5 w-5 text-purple-500" />} />}
          </div>
        ) : (
          ...
          <Card className="shadow-lg text-center p-8">
            ...
            <CardContent>
              <p className="text-gray-500">
                Le graphique des revenus est masqué. Vous pouvez le réactiver dans les <Link to="/admin/settings" className="font-semibold underline">paramètres</Link>.
              </p>
            </CardContent>
          </Card>
        )}
