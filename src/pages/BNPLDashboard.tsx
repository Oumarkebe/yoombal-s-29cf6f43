...
import { Loader2, AlertTriangle, Clock, Wallet, CalendarClock, FileText, Package, CheckCircle, BadgeCheck, XCircle, CircleSlash2 } from "lucide-react";
import { Alert, AlertTitle, AlertDescription } from "@/components/ui/alert";

const statusConfig: Record<string, { label: string; color: string; icon: React.ElementType; borderColor: string; }> = {
  active: { label: "Actif", color: "text-green-700 bg-green-50 border-green-200", icon: CheckCircle, borderColor: "border-green-500" },
  paid: { label: "Payé", color: "text-emerald-700 bg-emerald-50 border-emerald-200", icon: BadgeCheck, borderColor: "border-emerald-500" },
  overdue: { label: "Retard", color: "text-red-700 bg-red-50 border-red-200", icon: XCircle, borderColor: "border-red-500" },
  cancelled: { label: "Annulé", color: "text-gray-700 bg-gray-50 border-gray-200", icon: CircleSlash2, borderColor: "border-gray-500" }
};

const getStatus = (status: string) => {
  return statusConfig[status] || { label: status, color: "text-gray-700 bg-gray-50 border-gray-200", icon: CircleSlash2, borderColor: "border-gray-500" };
};
...
export default function BNPLDashboard() {
  const { plans, isLoading, error } = useBNPLPlans();
  ...
  const summary = React.useMemo(() => {
    return plans.reduce(
      ...
      { totalOutstanding: 0, activePlansCount: 0, nextPayment: { date: null as Date | null, amount: 0 } }
    );
  }, [plans]);
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-96 bg-gray-50 dark:bg-gray-900">
        ...
        <span className="text-lg text-gray-700 dark:text-gray-300">Chargement de vos paiements échelonnés...</span>
      </div>
    );
  }
    ...
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen py-8">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <header className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white tracking-tight">Mes Paiements Échelonnés</h1>
          <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">Suivez et gérez vos plans de paiement en toute simplicité.</p>
        </header>
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{summary.nextPayment.date ? formatCurrency(summary.nextPayment.amount) : "N/A"}</div>
                  <p className="text-xs text-muted-foreground">
                    {summary.nextPayment.date ? `Le ${summary.nextPayment.date.toLocaleDateString("fr-FR")}` : "Aucune échéance à venir"}
                  </p>
                </CardContent>
              </Card>
              <Card>
        ...
        {plans.length === 0 ? (
          <div className="text-center py-16 px-4 bg-white dark:bg-gray-800/50 rounded-lg shadow-sm">
            ...
            <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mb-2">Aucun plan de paiement trouvé</h3>
          ...
          <div className="space-y-6">
            {plans.map(plan => {
              ...
              return (
                <Card key={plan.id} className={`overflow-hidden transition-all hover:shadow-md border-l-4 ${statusInfo.borderColor}`}>
                  <CardHeader>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                      <div className="flex-1">
                        <CardTitle className="text-lg">Plan #{plan.id.slice(0, 8)}</CardTitle>
                        <CardDescription>Pour la commande #{plan.order_id.slice(0, 8)}</CardDescription>
                      </div>
                    ...
                    <div className="space-y-4 md:col-span-2">
                       <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-semibold text-gray-800 dark:text-gray-200">{formatCurrency(plan.total_amount)}</p>
                        </div>
                        <div>
                          <p className="text-gray-500 dark:text-gray-400">Mensualité</p>
                          <p className="font-semibold text-gray-800 dark:text-gray-200">{formatCurrency(plan.monthly_payment)}</p>
                        </div>
                        <div>
                          <p className="text-gray-500 dark:text-gray-400">Prochaine échéance</p>
                          <p className={`font-semibold ${plan.status === 'overdue' ? 'text-red-600' : 'text-gray-800 dark:text-gray-200'}`}>
                            {plan.next_payment_date ? new Date(plan.next_payment_date).toLocaleDateString("fr-FR") : "—"}
                          </p>
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between items-center mb-1 text-sm">
                          ...
                          <span className="font-medium text-gray-800 dark:text-gray-200">{plan.remaining_months} échéances restantes</span>
                        </div>
                    ...
                    <div className="flex flex-col items-start md:items-end justify-center md:border-l md:pl-6 border-gray-200 dark:border-gray-700">
                      {plan.status === 'overdue' && (
                        <div className="w-full">
                          <p className="text-red-600 font-bold mb-2 text-sm">⚠️ Paiement en retard !</p>
                          <Button className="w-full bg-red-600 hover:bg-red-700">Régulariser maintenant</Button>
                        </div>
                      )}
                       {plan.status === 'active' && (
                         <Button className="w-full md:w-auto">Effectuer un paiement</Button>
                       )}
                       {plan.status === 'paid' && (
                         <p className="text-sm text-emerald-600 font-medium">Plan entièrement remboursé.</p>
                       )}
                    </div>
                  </CardContent>
                </Card>
