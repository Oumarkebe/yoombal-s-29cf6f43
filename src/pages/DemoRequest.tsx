...
const GUIDE_SCENARIOS: Record<string, GuideStep[]> = {
  merchant: [
    {
      targetId: 'tour-merchant-inputs',
      title: 'Paramétrez votre Boutique',
      content:
        'Définissez votre volume de commandes et panier moyen. Ces sliders impactent tout le reste de la simulation.',
      position: 'right',
    },
    {
      targetId: 'tour-merchant-revenue',
      title: 'Projection Financière',
      content: 'Visualisez instantanément votre CA mensuel potentiel projeté sur 30 jours.',
      position: 'left',
    },
    {
      targetId: 'tour-merchant-ai',
      title: 'Générateur IA',
      content:
        'Testez notre IA générative : elle crée titre, description et prix optimisé en un clic.',
      position: 'bottom',
    },
    {
      targetId: 'tour-merchant-ads',
      title: 'Marketing Automatisé',
      content:
        "Simulez le ROI de vos campagnes. L'IA ajuste le budget pour maximiser la conversion.",
      position: 'top',
    },
  ],
  delivery: [
    {
      targetId: 'tour-delivery-map',
      title: 'Tracking Flotte',
      content:
        'Vue en temps réel de vos livreurs. Les points verts sont vos agents actifs sur le terrain.',
      position: 'right',
    },
    {
      targetId: 'tour-delivery-stats',
      title: 'KPIs Logistiques',
      content: "Suivez le taux d'optimisation et le volume horaire pour gérer vos pics d'activité.",
      position: 'top',
    },
    {
      targetId: 'tour-delivery-vehicle',
      title: 'Choix du Véhicule',
      content: 'Comparez la rentabilité entre Vélo, Moto et Fourgon selon vos trajets.',
      position: 'left',
    },
  ],
  client: [
    {
      targetId: 'tour-client-bnpl',
      title: 'Scoring IA',
      content:
        'Essayez le moteur de décision de crédit. Il analyse le risque en temps réel pour accorder le BNPL.',
      position: 'left',
    },
    {
      targetId: 'tour-client-features',
      title: 'Expérience Achat',
      content:
        'Découvrez les fonctionnalités premium offertes à vos clients (Recherche visuelle, Suivi...).',
      position: 'right',
    },
  ],
  global: [
    {
      targetId: 'tour-sim-controls',
      title: 'Moteur de Simulation',
      content: 'Lancez la simulation automatique ici. Tout le dashboard prendra vie.',
      position: 'bottom',
    },
    {
      targetId: 'tour-chaos-btn',
      title: 'Fail-Safe Test',
      content:
        "Le bouton rouge déclenche une crise majeure (Chaos). Observez comment l'IA réagit via le journal et modifie les courbes.",
      position: 'bottom',
    },
    {
      targetId: 'tour-timeline',
      title: 'Cerveau IA',
      content: "Ce journal affiche toutes les décisions prises par l'IA en temps réel.",
      position: 'left',
    },
  ],
};
          ...
          <CardHeader className="pb-2">
            <div className="flex justify-between items-start">
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-6 w-6 -mr-2 text-slate-500 hover:text-white"
                onClick={onClose}
              >
                <X className="h-4 w-4" />
              </Button>
            ...
            <div className="flex justify-between items-center pt-2 border-t border-white/5 mt-4">
              <div className="flex gap-1">
                {steps.map((_, i) => (
                  <div
                    key={i}
                    className={`h-1.5 rounded-full transition-all ${i === currentStep ? 'w-6 bg-amber-500' : 'w-1.5 bg-slate-700'}`}
                  />
                ))}
              </div>
    ...
    <Card className="bg-slate-950 border-slate-800 h-full flex flex-col" id="tour-timeline">
      <CardHeader className="py-3 border-b border-slate-800">
        <CardTitle className="text-sm font-mono text-slate-400 flex items-center gap-2">
          <Bot className="h-4 w-4 text-purple-500" /> JOURNAL SYSTÈME IA
        </CardTitle>
      </CardHeader>
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-4 space-y-3 font-mono text-xs max-h-[300px] lg:max-h-none"
      >
        {events.length === 0 && (
          <div className="text-slate-600 italic">En attente de démarrage...</div>
        )}
        {events.map((e, i) => (
          <div
            key={i}
            className={`flex gap-3 animate-in fade-in slide-in-from-left-2 duration-300 ${e.level === 'critical' ? 'text-red-400 bg-red-950/20 p-2 rounded border border-red-900/50' : e.level === 'warning' ? 'text-amber-400' : 'text-slate-300'}`}
          >
            <span className="opacity-50 select-none">[{e.time}]</span>
            <span>{e.message}</span>
          </div>
        ))}
      </div>
...
const BNPLSimulatorWidget = ({
  ...
  return (
    <Card
      className="border-amber-100 bg-amber-50/50 backdrop-blur-sm h-full flex flex-col justify-between"
      id="tour-client-bnpl"
    >
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Shield className="h-5 w-5 text-amber-600" /> Simulateur Scoring Crédit IA
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="space-y-4">
          <div className="flex justify-between">
            <Label>Montant du panier</Label>
            <span className="font-bold text-lg text-amber-700">{amount[0].toLocaleString()} F</span>
          </div>
          <Slider
            value={amount}
          ...
          <Button
            ...
            disabled={loading}
            className="w-full bg-amber-600 hover:bg-amber-700 text-white shadow-lg shadow-amber-200"
          >
            {loading ? <Loader2 className="animate-spin mr-2" /> : 'Lancer Scoring'}
          </Button>
        </div>
        {result && (
          <div className="bg-white p-4 rounded-xl border border-amber-100 shadow-sm animate-in zoom-in duration-300">
            <div className="flex justify-between items-center mb-4">
              <span className="text-sm text-slate-500">Décision IA</span>
              <Badge className={result.decision === 'ACCEPTÉ' ? 'bg-green-600' : 'bg-red-600'}>
                {result.decision}
              </Badge>
            </div>
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <div className="text-xs text-slate-400">Score de risque</div>
                <div className="text-2xl font-bold text-slate-900">{result.score}/100</div>
              </div>
              <div>
                <div className="text-xs text-slate-400">Indice Confiance</div>
...
const MerchantAISimulator = ({ autoMode }: { autoMode: boolean }) => {
  ...
  return (
    <Card
      className="border-slate-200 bg-white shadow-md overflow-hidden h-full flex flex-col"
      id="tour-merchant-ai"
    >
      <CardHeader className="bg-slate-900 text-white">
        <CardTitle className="flex items-center gap-2">
          <Bot className="h-5 w-5 text-purple-400" /> Générateur Fiche Produit IA
        </CardTitle>
        <CardDescription className="text-slate-400">
          Tapez un nom de produit basique, notre IA fait le reste.
        </CardDescription>
      </CardHeader>
      <CardContent className="p-6 space-y-6 flex-1 flex flex-col">
        <div className="flex gap-4">
          <Input
            placeholder="Ex: Baskets Rouges"
            value={productName}
          ...
          >
            {isGenerating ? (
              ...
              <>
                <Sparkles className="mr-2 h-4 w-4" /> Go
              </>
            )}
          </Button>
        </div>
        {generatedResult && (
          <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-500 rounded-xl border border-purple-100 bg-purple-50/50 p-4 flex-1">
            <div className="space-y-1">
              <Label className="text-xs text-purple-600 font-bold uppercase">Titre Optimisé</Label>
              <div className="font-semibold text-slate-900 text-sm">{generatedResult.title}</div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-white p-2 rounded border border-slate-100 shadow-sm text-center">
                <Label className="text-[10px] text-slate-400">Marché</Label>
                <div className="font-mono text-slate-500 line-through text-xs">
                  {generatedResult.price}
                </div>
              </div>
              <div className="bg-green-50 p-2 rounded border border-green-100 shadow-sm text-center">
                <Label className="text-[10px] text-green-600 font-bold flex items-center justify-center gap-1">
                  <Zap className="h-3 w-3" /> IA
                </Label>
                <div className="font-bold text-green-700 text-sm">
                  {generatedResult.optimizedPrice}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-auto pt-4">
          <Button
            ...
            asChild
          >
            <Link to="/pricing?plan=growth">Essayer l'Assistant Marchand</Link>
          </Button>
        </div>
      </CardContent>
    </Card>
    ...
    <Card className="border-slate-200 bg-white shadow-md">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg flex items-center gap-2">
          <Box className="h-5 w-5 text-orange-500" /> Prédictif de Stock (IA)
        </CardTitle>
          ...
          {chaosStockLevel < 1 && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white/90 backdrop-blur p-4 rounded-xl shadow-lg border border-red-500 text-center animate-in zoom-in duration-500 z-10 w-[80%]">
              <div className="flex items-center justify-center gap-2 text-red-600 font-bold mb-1">
                <TrendingUp className="h-4 w-4" /> RUPTURE IMMINETE
              </div>
              <Button
                size="sm"
                className="bg-red-600 hover:bg-red-700 text-white w-full animate-pulse"
              >
                Commander Express (IA)
              </Button>
            </div>
          )}
...
const AdsSimulator = ({ budget }: { budget: number }) => {
  ...
  return (
    <Card className="border-slate-200 bg-white shadow-md" id="tour-merchant-ads">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg flex items-center gap-2">
          <Megaphone className="h-5 w-5 text-pink-500" /> Simulateur Publicitaire
        </CardTitle>
        <CardDescription>Estimez portée vs budget.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
...
const DemoHub = () => {
  ...
  const [engine] = useState(() =>
    createSimulationEngine({ speed: 1, chaosLevel: 0.1, objective: 'growth' })
  );
  ...
  const handleChaos = () => {
    ...
    if (effect) {
      if (effect.target === 'orders') {
        ...
        setDeliveryEfficiency(0.4); // Traffic jam
      }
        ...
        <div className="sticky top-16 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm transition-all duration-300">
          <div className="max-w-7xl mx-auto px-4 py-3 flex flex-wrap items-center justify-between gap-4">
            ...
            >
              ...
              >
                {simulationActive ? (
                  ...
                  <>
                    <Play className="mr-2 h-4 w-4" /> Lancer Simulation
                  </>
                )}
              </Button>
              ...
              {[1, 2, 5].map((speed) => (
                <button
                  ...
                  className={`px-3 py-1 rounded text-xs font-bold transition-all ${simSpeed === speed ? 'bg-white shadow text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  x{speed}
                </button>
              ))}
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={startGuide} className="hidden md:flex">
                <HelpCircle className="mr-2 h-4 w-4" /> Guide Interactif
              </Button>
              <Button
                variant="destructive"
                size="sm"
                ...
                id="tour-chaos-btn"
              >
                <AlertTriangle className="mr-2 h-4 w-4" /> CRISE
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 grid lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {/* KPIS REACT TO STATE */}
              <Card
                className={`border-slate-100 shadow-sm transition-all ${chaosMultiplier > 1.5 ? 'bg-red-50 border-red-200 scale-105' : ''}`}
              >
              ...
              <Card className="border-slate-100 shadow-sm">
                <CardContent className="p-4 flex flex-col items-center text-center">
                  <Store className="h-6 w-6 mb-2 text-purple-600" />
                  <div className="text-2xl font-bold text-slate-900">
                    {(avgOrderValue[0] / 1000).toFixed(1)}k
                  </div>
                  <div className="text-xs text-slate-500 uppercase font-medium">Panier Moyen</div>
                </CardContent>
              </Card>
              <Card className="border-slate-100 shadow-sm">
                <CardContent className="p-4 flex flex-col items-center text-center">
                  <Megaphone className="h-6 w-6 mb-2 text-pink-600" />
                  <div className="text-2xl font-bold text-slate-900">
                    {(adsBudget[0] / 1000).toFixed(1)}k
                  </div>
                  <div className="text-xs text-slate-500 uppercase font-medium">Budget Pub</div>
                </CardContent>
              </Card>
              <Card className="border-slate-100 shadow-sm">
                <CardContent className="p-4 flex flex-col items-center text-center">
                  ...
                  <div className="text-2xl font-bold text-slate-900">
                    {((dailyOrders[0] * avgOrderValue[0] * 30) / 1000000).toFixed(2)}M
                  </div>
                  <div className="text-xs text-slate-500 uppercase font-medium">CA Mensuel</div>
                </CardContent>
              </Card>
            </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <TabsList className="bg-white border border-slate-200 shadow-sm w-full justify-start p-1 h-auto">
                <TabsTrigger value="merchant" className="px-6 py-2.5">
                  Espace Marchand
                </TabsTrigger>
                <TabsTrigger value="client" className="px-6 py-2.5">
                  Crédit IA (BNPL)
                </TabsTrigger>
                <TabsTrigger value="delivery" className="px-6 py-2.5">
                  Hyper-Logistique
                </TabsTrigger>
              </TabsList>
              ...
              >
                <div className="grid md:grid-cols-2 gap-6">
                  <Card className="h-full border-slate-200 shadow-md">
                    <CardHeader>
                      <CardTitle>Marchand : Revenus & Pubs</CardTitle>
                      <CardDescription>
                        Corrélation automatique Budget Pub / Commandes
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                      <div className="space-y-3" id="tour-merchant-inputs">
                        <div className="flex justify-between text-sm">
                          <span className="text-slate-600">Commandes Quotidiennes</span>{' '}
                          <b>{dailyOrders[0]}</b>
                        </div>
                        <Slider
                          value={dailyOrders}
                          onValueChange={setDailyOrders}
                          min={0}
                          max={100}
                          className="py-2"
                          disabled={simulationActive}
                      ...
                      <div
                        className="h-40 mt-4 bg-slate-50 rounded-xl border border-slate-100 flex items-end justify-between px-4 pb-2 overflow-hidden relative"
                        id="tour-merchant-revenue"
                      >
                        {Array.from({ length: 12 }).map((_, i) => {
                          const height = Math.min(
                            100,
                            Math.max(10, dailyOrders[0] + Math.sin(i) * 10 + Math.random() * 20)
                          );
                          return (
                            <div
                              key={i}
                              className={`w-[6%] rounded-t-sm transition-all duration-500 ${chaosMultiplier > 1.5 ? 'bg-red-500' : 'bg-blue-500'}`}
                              style={{ height: `${height}%` }}
                            ></div>
                          );
                        })}
                  ...
                  <div className="space-y-8 p-4" id="tour-client-features">
                    ...
                    <ul className="space-y-4">
                      <li className="flex items-center gap-2">
                        <CheckCircle2 className="text-green-500" /> Recherche Visuelle
                      </li>
                      <li className="flex items-center gap-2">
                        <CheckCircle2 className="text-green-500" /> Suivi Livreur Live
                      </li>
                      <li className="flex items-center gap-2">
                        <CheckCircle2 className="text-green-500" /> Chatbot Support IA 24/7
                      </li>
                    </ul>
                  </div>
                </div>
              ...
              <TabsContent value="delivery" className="animate-in fade-in slide-in-from-bottom-4">
                <div className="grid md:grid-cols-2 gap-6">
                  <Card
                    className="h-full border-slate-200 shadow-md overflow-hidden"
                    id="tour-delivery-map"
                  >
                    <CardHeader className="bg-slate-900/5 pb-4">
                      <CardTitle className="flex items-center gap-2">
                        <MapPin className="h-5 w-5 text-emerald-600" /> Live Fleet Tracking
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="p-0 relative h-[350px] bg-slate-900">
                      ...
                      <div
                        className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] border rounded-full animate-ping [animation-duration:3s] ${deliveryEfficiency < 0.5 ? 'border-red-500/50' : 'border-emerald-500/20'}`}
                      ></div>
                      {Array.from({ length: 8 }).map((_, i) => (
                        <div
                          ...
                          style={{
                            ...
                            top: `${simulationActive ? 20 + Math.random() * 60 : 50}%`,
                            left: `${simulationActive ? 20 + Math.random() * 60 : 50 + i * 5 - 20}%`,
                          }}
                        >
                          {/* Driver Dot */}
                          <div
                            className={`relative w-3 h-3 rounded-full shadow-[0_0_10px_currentColor] cursor-pointer ${deliveryEfficiency < 0.5 ? 'bg-red-500 text-red-500' : 'bg-emerald-500 text-emerald-500'}`}
                          >
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 bg-slate-800 text-white text-[10px] p-2 rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity z-20 pointer-events-none border border-slate-700">
                            ...
                            <div className="flex items-center gap-1 text-slate-400">
                              <Battery className="h-3 w-3" /> {85 - i * 5}%
                            </div>
                            <div className="flex items-center gap-1 text-slate-400">
                              <Signal className="h-3 w-3" /> 4G Good
                            </div>
                          </div>
                        </div>
                      ))}
                      <div
                        ...
                        id="tour-delivery-stats"
                      >
                        <div className="bg-slate-800/90 backdrop-blur p-2 rounded-lg text-center border border-slate-700">
                          <div className="text-xs text-slate-400">Livreurs</div>
                          <div className="font-bold text-emerald-400">
                            {simulationActive ? '142' : '15'}
                          </div>
                        </div>
                        <div className="bg-slate-800/90 backdrop-blur p-2 rounded-lg text-center border border-slate-700">
                          <div className="text-xs text-slate-400">Cmd/h</div>
                          <div className="font-bold text-blue-400">
                            {simulationActive ? Math.round(850 * chaosMultiplier) : '0'}
                          </div>
                        </div>
                        <div className="bg-slate-800/90 backdrop-blur p-2 rounded-lg text-center border border-slate-700">
                          <div className="text-xs text-slate-400">Optim.</div>
                          <div
                            className={`font-bold ${deliveryEfficiency < 0.5 ? 'text-red-500' : 'text-purple-400'}`}
                          >
                            {(deliveryEfficiency * 100).toFixed(1)}%
                          </div>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  <Card className="h-full border-slate-200 shadow-md" id="tour-delivery-vehicle">
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                        <Car className="h-5 w-5 text-blue-600" /> Comparatif Véhicules
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="flex flex-col h-full">
                      <div className="space-y-4 flex-1">
                        {[
                          { name: 'Moto', val: 450000, color: 'bg-emerald-500' },
                          { name: 'Vélo', val: 150000, color: 'bg-blue-500' },
                          { name: 'Fourgon', val: 1200000, color: 'bg-purple-500' },
                        ].map((v) => (
                          <div key={v.name} className="space-y-1">
                            <div className="flex justify-between text-sm">
                              <span>{v.name}</span> <span>{v.val.toLocaleString()} F</span>
                            </div>
                            <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full ${v.color}`}
                                style={{ width: `${v.val / 15000}%` }}
                              ></div>
                            </div>
                          </div>
                        ))}
                        <p className="text-xs text-slate-400 mt-4">
                          Revenu net mensuel estimé après déduction carburant.
                        </p>
                      </div>
                      <div className="pt-6">
                        <Button
            ...
            <div className="bg-gradient-to-br from-violet-600 to-indigo-700 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
              <div className="relative z-10">
                <h3 className="font-bold text-lg mb-2 flex items-center gap-2">
                  <Sparkles className="h-4 w-4 text-yellow-300" /> Yoombal Vision
                </h3>
                <p className="text-indigo-100 text-sm mb-4">
                  L'IA analyse en permanence 140+ points de données pour optimiser votre business.
                </p>
                <div className="space-y-2">
                  {[
                    'Analyse de la concurrence locale',
                    'Météo & Impact Livraison',
                    'Tendances de recherche Google',
                    'Solvabilité micro-crédit',
                  ].map((item, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 text-xs bg-white/10 p-2 rounded"
                    >
