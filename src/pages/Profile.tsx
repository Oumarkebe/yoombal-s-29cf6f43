...
const Profile = () => {
  ...
  const handleDeleteOrder = async (orderId: string) => {
    if (
      !window.confirm(
        'Êtes-vous sûr de vouloir supprimer cette commande de votre historique ? Cette action est irréversible.'
      )
    ) {
      return;
    }
              ...
              <div className="mt-6 pt-6 border-t">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="business_type" {...form.register('business_type')} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="vehicle_type">Type de véhicule (livreurs)</Label>
                    <Input id="vehicle_type" {...form.register('vehicle_type')} />
                  </div>
                </div>
                ...
                <div className="mt-6 pt-6 border-t space-y-4">
                  ...
                  <p className="text-sm text-gray-500">
                    Donnez un nom spécifique à chacun de vos profils
                  </p>
            ...
            <Card className="p-6">
              ...
              <div className="space-y-4">
                {ordersLoading ? (
                  ...
                  orders.map((order) => {
                    const digitalItems =
                      order.order_items?.filter((item: any) => item.products?.is_digital) || [];
                    ...
                    return (
                      ...
                      >
                        ...
                        {digitalItems.length > 0 &&
                          ['paid', 'delivered', 'completed', 'shipped'].includes(
                            ...
                            return (
                              <div className="mt-2 pt-2 border-t space-y-2">
                                ...
                                {digitalItems.map((item: any, idx: number) => (
                                  <div
                                    key={idx}
                                    className="flex items-center justify-between text-sm bg-emerald-50 p-2 rounded"
                                  >
                                    <span className="truncate mr-2">{item.products.name}</span>
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-7 text-emerald-700 hover:text-emerald-800 hover:bg-emerald-100 p-0 px-2"
                                      asChild
                                    >
                                      <a
                                        href={formatDownloadUrl(item.products.download_url)}
                                        target="_blank"
              ...
              <div className="space-y-6">
                {bnplLoading ? (
                  ...
                  bnplPlans.map((plan: any) => (
                    <div
                      key={plan.id}
                      className="p-5 border border-orange-200 rounded-xl bg-gradient-to-br from-white to-orange-50"
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                          {plan.products?.image_url && (
                            <img
                              src={plan.products.image_url}
                              alt=""
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                          {plan.installments?.map((inst: any, idx: number) => (
                            <div
                              key={idx}
                              className={`flex items-center justify-between p-2 rounded-lg text-sm border ${inst.status === 'paid'
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <div
                                  className={`w-2 h-2 rounded-full ${inst.status === 'paid' ? 'bg-emerald-500' : 'bg-amber-400'}`}
                                />
                                <span
                                  className={
                                    inst.status === 'paid'
                      ...
                      {plan.status === 'awaiting_deposit' && (
                        <div className="mt-6 p-4 bg-amber-50 rounded-lg border border-amber-200 text-center">
                          <p className="text-sm text-amber-800 mb-3 font-medium">
                            Votre demande a été approuvée ! Finalisez l'apport initial pour activer
                            votre crédit.
                          </p>
                          <Button
                          ...
                          >
                            Payer l'apport initial (
                            {new Intl.NumberFormat('fr-SN', {
                              style: 'currency',
                              currency: 'XOF',
                            }).format(
                              plan.installments?.find((i: any) => i.type === 'deposit')?.amount || 0
                            )}
                            )
                          </Button>
                        </div>
