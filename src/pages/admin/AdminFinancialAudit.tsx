...
export default function AdminFinancialAudit() {
  ...
  const totalDiscrepancy =
    auditData?.reduce((acc, a) => acc + ((a.received_amount || 0) - a.order_amount), 0) || 0;
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Vérification de la concordance entre les ordres Yoombal et les paiements réels.
          </p>
        </div>
                  ...
                  <tr key={audit.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-4 font-medium">
                      ...
                      <div className="text-[10px] text-slate-400">
                        {audit.orders?.profiles
                          ? `${audit.orders.profiles.first_name} ${audit.orders.profiles.last_name}`
                          : 'Acheteur Inconnu'}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 text-slate-500 text-xs">{audit.notes || '-'}</td>
                  </tr>
                ))}
