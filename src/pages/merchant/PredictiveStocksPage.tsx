...
const MOCK_STOCK_ITEMS = [
  {
    id: 1,
    name: 'Oignons Import (Sac 25kg)',
    stock: 12,
    salesTrend: [10, 15, 20, 25, 40, 55, 60],
    status: 'critical',
    prediction: 'Rupture dans 3j',
  },
  {
    id: 2,
    name: 'Smartphone Y80',
    stock: 145,
    salesTrend: [5, 4, 3, 2, 2, 1, 0],
    status: 'dormant',
    prediction: 'Stock mort',
  },
  {
    id: 3,
    name: 'Riz Parfumé Royal',
    stock: 200,
    salesTrend: [50, 52, 48, 55, 60, 65, 80],
    status: 'healthy',
    prediction: 'Demande en hausse',
  },
  {
    id: 4,
    name: 'Ventilateur Stand',
    stock: 5,
    salesTrend: [2, 5, 8, 12, 15, 18, 20],
    status: 'opportunity',
    prediction: 'Pic de chaleur',
  },
];
...
export default function PredictiveStocksPage() {
    ...
    switch (status) {
      ...
      case 'dormant':
        return (
          <Badge
            variant="secondary"
            className="bg-orange-100 text-orange-700 hover:bg-orange-200 border-none flex gap-1"
          >
            <Coins className="w-3 h-3" /> Dormant
          </Badge>
        );
      case 'opportunity':
        return (
          <Badge variant="default" className="bg-green-600 hover:bg-green-700 flex gap-1">
            <TrendingUp className="w-3 h-3" /> Opportunité
          </Badge>
        );
      default:
        return (
  ...
  const getSmartAction = (item: any) => {
    switch (item.status) {
      case 'critical':
        return (
          <Button size="sm" className="bg-red-600 hover:bg-red-700 text-white h-7 text-xs w-full">
            Commander +100
          </Button>
        );
      case 'dormant':
        return (
          <Button
            size="sm"
            variant="outline"
            className="text-orange-600 border-orange-200 hover:bg-orange-50 h-7 text-xs w-full"
          >
            Promo -20%
          </Button>
        );
      case 'opportunity':
        return (
          <Button
            size="sm"
            className="bg-green-600 hover:bg-green-700 text-white h-7 text-xs w-full"
          >
            Recommender Stock
          </Button>
        );
      default:
        return (
          <span className="text-xs text-gray-400 text-center block">Aucune action requise</span>
        );
    }
  };
        ...
        <main className="flex-grow container mx-auto px-4 py-8 max-w-7xl">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex items-center gap-4 bg-white p-2 pr-6 rounded-full shadow-sm border">
              <div className="bg-green-100 text-green-700 font-bold p-3 rounded-full text-lg">
                85/100
              </div>
              <div>
            ...
            <Card className="bg-gradient-to-br from-indigo-900 to-indigo-800 text-white border-none shadow-lg overflow-hidden relative">
              ...
              <CardHeader className="pb-2">
                <div className="flex justify-between items-center">
                  <CardTitle className="text-lg font-light opacity-90">
                    Contexte Saisonnier
                  </CardTitle>
                  <Badge className="bg-indigo-500 hover:bg-indigo-600 border-none">
                    J-{MOCK_SEASONALITY.daysLeft}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
                ...
                <div className="bg-white/10 p-3 rounded-lg backdrop-blur-sm">
                  <p className="text-xs text-indigo-100 mb-2 uppercase font-semibold">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {MOCK_SEASONALITY.suggestedItems.map((i) => (
                      <Badge
                        key={i}
                        variant="secondary"
                        className="bg-white text-indigo-900 hover:bg-indigo-50 text-[10px]"
                      >
                        {i}
                      </Badge>
                    ))}
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-5">
                ...
                <p className="text-sm text-indigo-800/80 mb-4">
                  Notre IA analyse vos mouvements de stock pour détecter les anomalies et
                  opportunités.
                </p>
                ...
                <div className="space-y-4">
                  <div className="bg-white p-3 rounded-lg shadow-sm">
                    ...
                    <p className="text-xs text-gray-500">
                      Produits non vendus depuis +60 jours. Ils vous coûtent de l'argent en
                      stockage. Liquidez-les !
                    </p>
                  </div>
                  ...
                  <div className="bg-white p-3 rounded-lg shadow-sm">
                    ...
                    <p className="text-xs text-gray-500">
                      L'IA croise vos ventes avec le calendrier local (fêtes religieuses, climat)
